...
def draw_curved_rectangle(frame, x, y, w, h, corner_radius, thickness=2, color=( 128, 128,0)):
    """
...
def get_packet_id(file_path):
    ...
    try:
        ...
        title = metadata.get('format', {}).get('tags', {}).get('title', 'Title not found')
        return title
...
class AccountScraper:
    ...
    def __init__(self,  dates = None, mac_path = None, mac_id = None, acc_id = None):
        ...
        if dates is None:
            dates = pd.date_range(start=pd.to_datetime('today') - pd.Timedelta(days=7), end=pd.to_datetime('today'))
    ...
    def download_logs(self, dates):
        ...
        for log_path_ in log_path:
            ...
            if 'log_analyzer' not in  '/'.join(log_path_.split('/')[:-2]):
                subprocess.run(['rm', '-rf', '/'.join(log_path_.split('/')[:-2])])
        ...
        with open(self.log_paths[0]) as f:
            for line in tqdm.tqdm(f):
                ...
                if 'MONITORING' in line:
                    ...
                    rate_index = next((i for i, s in enumerate(line_splitted) if 'rate:' in s), None)
    ...
    def get_detector_confidence(self, monitoring = True):
        ...
        for line in self.lines:
            ...
            conf_index = next((i for i, s in enumerate(line_splitted) if 'subj: ' in s), None)
    ...
    def get_power(self, monitoring = True):
        ...
        for line in tqdm.tqdm(self.lines):
            ...
            # Find the index containing 'subj:'
            pow_index = next((i for i, s in enumerate(line_splitted) if 'pow: ' in s), None)
    ...
    def get_breathing_classifier(self, breathing_only =False):
        ...
        # Create a DataFrame from the collected data
        df = pd.DataFrame(data, columns=['time', 'bc'])
        df['time'] = pd.to_datetime(df['time'])
    ...
    def get_classifier_counter(self, df_bc):
        ...
        df_bc['bc'] = df_bc['bc'].rolling(10).quantile(0.5)
        df_state = self.get_state()
        df_counter = df_bc
        df_counter['no_breathing_counter'] = df_counter['bc'] > 0.5
        df_counter['no_breathing_counter'] = df_counter['no_breathing_counter'].astype(int)

        df_bc.at[0, 'no_breathing_counter'] = 0
        for i in range(1, len(df_counter)):
            if df_counter.at[i, 'no_breathing_counter'] == 1:
                df_counter.at[i, 'no_breathing_counter'] = df_counter.at[i - 1, 'no_breathing_counter'] + 20
            else:
                df_counter.at[i, 'no_breathing_counter'] = df_counter.at[i - 1, 'no_breathing_counter'] - 20

            if df_counter.at[i, 'no_breathing_counter'] < 0:
                df_counter.at[i, 'no_breathing_counter'] = 0
            curr_state = df_state[df_state['time'] < df_counter.at[i, 'time']].iloc[-1]['state']
            if curr_state != 'breathing micro':
                df_counter.at[i, 'no_breathing_counter'] = 0
        return df_counter

    def annotate_video(self, vid_file_path = None, play = False, output_path = None, vid_idx = None):
        ...
        for idx, _path in enumerate(vid_path_list):
            ...
            log_df['confidence_head'] = log_df['confidence_head'].fillna(method='ffill')
            ...
            while True:
                ...
                if len(prev_lines)>0:
                    ...
                    x, y, w, h = prev_lines['x'].values[0], prev_lines['y'].values[0], prev_lines['w'].values[0], prev_lines['h'].values[0]
                    x, y, w, h = int(x), int(y), int(w), int(h)
                    conf = prev_lines['confidence'].values[0]
                    # draw box
                    if w > 0 and h > 0:
                        frame = draw_curved_rectangle(frame, x, y, w, h, 10)
                        cv2.putText(frame, str(conf), (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

                    try:
                        x_head, y_head, w_head, h_head = prev_lines['x_head'].values[0], prev_lines['y_head'].values[0], prev_lines['w_head'].values[0], prev_lines['h_head'].values[0]
                        x_head, y_head, w_head, h_head = int(x_head), int(y_head), int(w_head), int(h_head)
                        conf_head = prev_lines['confidence_head'].values[0]
                        # draw box
                        if w_head > 0 and h_head > 0:
                            frame = draw_curved_rectangle(frame, x_head, y_head, w_head, h_head, 10)
                            cv2.putText(frame, str(conf_head), (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
                    except:
                        pass

                    # add state
                    cv2.putText(frame, prev_lines['state'].values[0], (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

                    # add the pwr
                    cv2.putText(frame, str(power.values[0]), (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)

                    # primary position
                    pp = prev_lines['primary_position'].values[0]
                    if pp is not None:
                        # add as a scatter
                        cv2.circle(frame, (int(pp[0]), int(pp[1])), 5, (0, 0, 255), -1)
                    ...
                    if len(prev_lines['notification']) > 0:
                        if prev_lines['notification'].values[0] is not None:
                            prev_notification = prev_lines['notification'].values[0]
                            # keep this prev notification for 2*20 frames
                    cv2.putText(frame, prev_lines['notification'].values[0], (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1,
    ...
    def get_videos(self):
        ...
        day_of_week = tdate.fromisoformat(str(year) + '-' + str(month) + '-' + str(day)).weekday()
        ...
        list_files = storage_client.list_blobs('production-nanobebe-video',prefix= str(year) + '/' + str(week) + '/' + str(day_of_week) + '/' + mac_id)
        list_vids = []
        triggers = []
        ...
        for vid in tqdm.tqdm(list_vids[:3]):
            blob = storage.Blob(vid, storage_client.bucket('production-nanobebe-video'))
            filename = root + 'temp_vids/' + vid.split('/')[-1]
            blob.download_to_filename(filename)
            paths.append(filename)
            vid_df.loc[vid_df['video'] == '/' + vid.split('/')[-1], 'local_path'] = filename
    ...
    def get_fields(self, fields = None, monitoring = True,new_fields = False):
        ...
        # init all dfs
        df = pd.DataFrame(columns=['time', 'confidence', 'bc1', 'bc2', 'roi', 'state', 'pow', 'notification', 'rate', 'bc_buffer', 'counter'])
        data = {'time': [], 'confidence': [], 'bc1': [], 'bc2': [], 'roi': [],
                'state': [], 'pow': [], 'notification': [], 'rate': [],
                'bc_buffer': [], 'counter': [],'monitoring': [], 'video' : [], 'focal': [] ,'pi': []}
        ...
        for idx, line in enumerate(self.lines):
            ...
            if 'ALGO_SEC' in line:
                ...
                self.df = df
                return df
            if 'MONITORING' not in line and  '+++++ NOTIFICATION +++++' not in line and 'rect' not in line and 'state' not in line and '.mp4' not in line:
                continue
            ...
            if vid_match:
                ...
                if 'uploadVideo' in next_line_split[0]:
                    try:
                        trigger = next_line_split[0].split(' ')[-2].split('[')[1][:-1]
            ...
            notif_index = next((i for i, s in enumerate(line_splitted) if '+++++ NOTIFICATION +++++' in s), None)
            if notif_index is not None:
                try:
                    notif = line_splitted[notif_index].split('+++++ NOTIFICATION +++++')[1]
            ...
            bc_index = next((i for i, s in enumerate(line_splitted) if 'bc: ' in s), None)
            ...
            roi_index = next((i for i, s in enumerate(line_splitted) if 'rect = ' in s), None)
            if roi_index is not None:
                try:
                    roi = list(map(int, line_splitted[roi_index].split(' ')[2].split(',')))
                    data['roi'].append(roi)
                except:
            ...
            primary_index_index = next((i for i, s in enumerate(line_splitted) if 'pi: ' in s), None)
            ...
            counter_index = next((i for i, s in enumerate(line_splitted) if 'bc: ' in s), None)
        ...
        for idx, line in enumerate(self.lines):
            ...
            if timestamp_match:
                ...
                if json_string:
                    ...
                    json_string, context = json_string.split(', "ctx":')[0], json_string.split(', "ctx":')[1]
    ...
    def plot_fields(self, fields = None, monitoring = True, classifier_breathing_only = True, save = None):
        df_conf = self.df.loc[self.df['confidence'].notnull()][['time', 'confidence']]
        ...
        for i in range(len(change_inds)):
            ...
            df = pd.concat(
                [df, pd.DataFrame({'state': [states.iloc[change_inds[i]]['state']], 'start': [start], 'end': [end]})])
            if classifier_breathing_only:
                if states.iloc[change_inds[i]]['state'] != 'breathing micro':
                    df_bc.loc[(df_bc['time'] > start) & (df_bc['time'] < end), 'bc1'] = None



        # create fig with seccondaary y axis
        tl_fig = px.timeline(df, x_start="start", x_end="end", y=pd.Series(np.ones(len(df)) - 0.6), color="state",
                             opacity=0.4)
        ...
        # add power
        tl_fig.add_trace(
            go.Scatter(x=df_pow.index, y=0.8 * df_pow["pow"] / df_pow['pow'].max(), name='power', hovertext=df_pow['pow'],
                       mode='lines', hovertemplate='Power: %{hovertext} \n time: %{x}'), row=1, col=1)

        tl_fig.add_trace(
            go.Scatter(x=df_bc_buffer.index, y=0.8 * df_bc_buffer["bc_buffer"] / df_bc_buffer['bc_buffer'].max(),
                          name='bc_counter', hovertext=df_bc_buffer['bc_buffer'], mode='lines',
                            hovertemplate='Breathing Classifier Buffer: %{hovertext} \n time: %{x}'), row=1, col=1)
        tl_fig.add_trace(
            go.Scatter(x=df_pi.index, y=0.8 * df_pi["pi"] / df_pi['pi'].max(),
                            name='pi', hovertext=df_pi['pi'], mode='lines',
                            hovertemplate='pi: %{hovertext} \n time: %{x}'), row=1, col=1)

        try:
            df_ap = self.df.loc[self.df['active_regions'].notnull()][['time', 'active_regions']]
            df_ap.set_index('time', inplace=True)
            tl_fig.add_trace(
                go.Scatter(x=df_ap.index, y=0.8 * df_ap["active_regions"] / df_ap['active_regions'].max(),
                            name='active_regions', hovertext=df_ap['active_regions'], mode='lines',
                            hovertemplate='active_regions: %{hovertext} \n time: %{x}'), row=1, col=1)
            tl_fig.update_traces(visible='legendonly', selector=dict(name="active_regions"))
        except:
            pass

        # add rate
        tl_fig.add_trace(
            go.Scatter(x=df_rate.index, y=0.8 * df_rate["rate"] / df_rate['rate'].max(), name='breathing_rate',
        ...
        tl_fig.add_trace(
            go.Scatter(x=df_mointoring.index, y=df_mointoring['y'], name='monitoring', mode='lines',
                          hovertemplate='time: %{x}', line=dict(color='black', width=4)), row=1, col=1)
        ...
        tl_fig.update_traces(visible='legendonly', selector=dict(name="bc_counter"))
        ...
        for i in range(len(notif_vals)):
            ...
            tl_fig.add_trace(go.Scatter(x=notif_df_total['time'], y=notif_df_total['y'], name=notif, mode='lines',
                                        hovertemplate='time: %{x}'), row=1, col=1)
            tl_fig.update_traces(visible='legendonly', selector=dict(name=notif))
        ...
        if plot_videos:
            ...
            for i in range(len(video_vals)):
                ...
                ## add 1 ms
                video_df2['time'] = video_df2['time'] + pd.Timedelta(milliseconds=1)
                video_df3['time'] = video_df3['time'] - pd.Timedelta(milliseconds=2)
                video_df3['y'] = None
                video_df2['y'] = 1
                video_df_total = pd.concat([video_df, video_df2, video_df3])
                video_df_total = video_df_total.sort_values(by=['time'])
                tl_fig.add_trace(go.Scatter(x=video_df_total['time'], y=video_df_total['y'], name=video, mode='lines',
                                            hovertemplate='time: %{x}'), row=1, col=1)
                tl_fig.update_traces(visible='legendonly', selector=dict(name=video))


        # add title with date and acc id and mac id
        tl_fig.update_layout(title_text=f"Date: {self.date} Acc ID: {self.acc_id} MAC ID: {self.mac_id}")
        tl_fig.show('browser')
        ...
        for line in self.lines:
            ...
            if version_match:
                ...
                if timestamp_match:
                    ...
                    time = f"{pd.to_datetime('today').year} {date} {timestamp}"
                    version_val = pattern.search(line).group().split('=')[1]
                    version_df = pd.concat([version_df, pd.DataFrame({'time': [time], 'version': [version_val]})])
        return version_df



    def get_all(self, vis = True, filter_notifs = ['breathingStopped'], get = ['conf', 'bc', 'pow', 'notif', 'state', 'roi', 'bc_buffer', 'rate']):
        ...
        if filter_notifs is not None:
            notifs = [notifs[notifs['notification'].str.contains(x)] for x in filter_notifs]
        ...
        if vis:
            ...
            for i in range(len(change_inds)):
                ...
                df = pd.concat([df, pd.DataFrame({'state': [states.iloc[change_inds[i]]['state']], 'start': [start], 'end': [end]})])
            ...
            # add other signals and set them toggled off
            tl_fig.add_trace(go.Scatter( x = df_conf.index, y=df_conf['confidence'] * 0.8, name='conf',hovertext=df_conf['confidence'], mode='lines', hovertemplate='confidence: %{hovertext} \n time: %{x}'), row=1, col=1)
            tl_fig.add_trace(go.Scatter(x=df_bc.index, y=0.8 * df_bc["bc"] / df_bc['bc'].max(), name='bc', hovertext=df_bc['bc'], mode='lines', hovertemplate='Breathing Classifier: %{hovertext} \n time: %{x}'), row=1, col=1)
            ...
            tl_fig.add_trace(go.Scatter(x=df_bc_buffer.index, y=0.8 * df_bc_buffer["bc_buffer"] / df_bc_buffer['bc_buffer'].max(), name='bc_counter', hovertext=df_bc_buffer['bc_buffer'], mode='lines', hovertemplate='Breathing Classifier Counter: %{hovertext} \n time: %{x}'), row=1, col=1)
            ...
            tl_fig.add_trace(go.Scatter(x=df_counter.index, y=0.8 * df_counter["no_breathing_counter"] / df_counter['no_breathing_counter'].max(), name='no_breathing_counter', hovertext=df_counter['no_breathing_counter'], mode='lines', hovertemplate='No Breathing Counter: %{hovertext} \n time: %{x}'), row=1, col=1)
            # toggle all signals except the first ogg
            tl_fig.update_traces(visible='legendonly', selector=dict(name="conf"))
            tl_fig.update_traces(visible='legendonly', selector=dict(name="bc"))
            tl_fig.update_traces(visible='legendonly', selector=dict(name="power"))
            tl_fig.update_traces(visible='legendonly', selector=dict(name="bc_counter"))
            tl_fig.update_traces(visible='legendonly', selector=dict(name="breathing_rate"))
            tl_fig.update_traces(visible='legendonly', selector=dict(name="no_breathing_counter"))
            ...
            for i in range(len(notif_vals)):
                ...
                notif_df_total = pd.concat([notif_df, notif_df2, notif_df3])
                notif_df_total = notif_df_total.sort_values(by=['time'])
                tl_fig.add_trace(go.Scatter(x=notif_df_total['time'], y=notif_df_total['y'], name=notif, mode='lines', hovertemplate='time: %{x}'), row=1, col=1)
                tl_fig.update_traces(visible='legendonly', selector=dict(name=notif))
            ...
            if html:
                ...
                fig = make_subplots(rows=2, cols=2, specs=[[{"colspan": 1, "secondary_y": True}, {"secondary_y": True}],
                                                           [{"secondary_y": True}, {"secondary_y": True}]],
                                    shared_xaxes=True, vertical_spacing=0.02, horizontal_spacing=0.02)
                fig.add_trace(px.line(pow, x="time", y="power").data[0], row=1, col=1)
                fig.add_trace(px.line(df_bc, x="time", y="bc").data[0], row=1, col=2)
                # title and grids
                ...
                fig.update_yaxes(title_text="Breathing Classifier", row=1, col=2)
                ...
                figures_to_html([tl_fig, fig], filename="dashboard.html")
    ...
    def get_fields_new(self, fields = None, monitoring = True, classifier_breathing_only = True):
        # Initialize an empty list to collect data
        ...
        for idx, line in enumerate(tqdm.tqdm(self.lines)):
            ...
            if  '+++++ NOTIFICATION +++++' not in line and  '.mp4' not in line and 'ALGO_SEC' not in line and 'ALGO_EVENT' not in line:
                continue
                ...
                if json_string:
                    ...
                    if "baby_detector_inference" in json_string:
                        ...
                        if 'confidence' in json_string:
                            confidence = re.search(r'confidence": (\d+\.\d\d)', line).group(1)
                        elif 'conf' in json_string:
                            confidence = re.search(r'conf": (\d+\.\d\d)', line).group(1)
                        data['confidence'][-1] = float(confidence)
                        rect = line.split('rect": [')[-1].split('}')[0].split('],')[0]
    ...
    def resolve_reason(self):
        ...
        df = self.df
        no_measure = df.loc[df['notification'] == '  monitoringStoppedNoMeasure\n']
        ...
        for nm in no_measure.iterrows():
            ...
            bc = df[(df['time'] > time - datetime.timedelta(minutes=5)) & (df['time'] < time)]['bc1']
            bc_counter = df[(df['time'] > time - datetime.timedelta(minutes=5)) & (df['time'] < time)]['bc_buffer']
            # check if the state had 'motion' in it in the past 3 minutes
            # get the state 3 minutes before
            state = df[(df['time'] > time - datetime.timedelta(minutes=3)) & (df['time'] < time)]['state'].values
            # was there state with motion in the past 3 minutes
            motion = any(['motion' in s for s in state if s is not None])
            prev_state = df[(df['time'] > time - datetime.timedelta(minutes=3)) & (df['time'] < time)]['state'].values[
                -5]
            # see what is the power
            pow = df[(df['time'] > time - datetime.timedelta(minutes=5)) & (df['time'] < time)]['pow']
            ...
            output = pd.concat([output, pd.DataFrame({'time': [time], 'reason': [reason]})])
